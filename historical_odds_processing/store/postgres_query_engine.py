...
class PostgresQueryEngine(PostgresEngine):
    ...
    def get_market(self, betfairEventId, betfairMarketId):
        ...
        if len(output) != 1:
            raise ValueError(f'No market info found for event: {betfairEventId}, market: {betfairMarketId} in tbl_betfair_markets')
        return output
    ...
    def get_runner_index(self, runnerName):
        output = self.run_select_query(
            query="""
                SELECT
                    id
        ...
        if len(output) == 1:
            ...
            raise ValueError(f'No single id found for {runnerName} in tbl_betfair_runners')
    ...
    def get_market_id(self, eventId, marketTypeId):
        ...
        if len(result) == 0:
            return None
        return result['betfair_market_id'][0]
    ...
    def get_event_id_start_time(self, betfairEventId):
        ...
        if len(result) == 0:
            return None
        return result['unix_timestamp'][0]
